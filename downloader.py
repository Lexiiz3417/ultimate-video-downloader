...
# ==============================================================================
# 1. PUSAT BAHASA (KAMUS UNTUK SEMUA TEKS)
# ==============================================================================
LANGS = {
...
# ==============================================================================
# 2. KELAS JENDELA LOADING GENERIK
# ==============================================================================
class DownloaderWindow(customtkinter.CTkToplevel):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs); self.title("Memuat Komponen..."); self.geometry("400x200"); self.resizable(False, False); self.protocol("WM_DELETE_WINDOW", self.on_closing); self.grab_set()
    ...
    def ytdlp_worker(self):
        ...
        try:
            ...
            bin_folder = os.path.join(get_app_support_path(), "bin"); ytdlp_path = os.path.join(bin_folder, "yt-dlp.exe")
            with requests.get(asset_url, stream=True, headers={'User-Agent': 'Mozilla/5.0'}, timeout=30) as r:
                ...
                with open(ytdlp_path, 'wb') as f:
                    ...
                    for chunk in r.iter_content(chunk_size=8192):
                        if chunk: f.write(chunk); dl_size += len(chunk)
                        if total_size > 0: progress = dl_size / total_size; self.update_queue.put(('progress', progress)); self.update_queue.put(('percent', f"{progress*100:.1f}%")); self.update_queue.put(('status_label', f"{dl_size/1024/1024:.2f} MB / {total_size/1024/1024:.2f} MB"))
    ...
    def ffmpeg_worker(self):
        application_path = get_app_support_path(); bin_folder = os.path.join(application_path, "bin"); zip_path = os.path.join(bin_folder, "ffmpeg.zip"); os.makedirs(bin_folder, exist_ok=True)
        try:
        ...
        except Exception as e: print(f"ERROR FFMPEG: {e}"); self.update_queue.put(('messagebox', ('error', "Error", f"Gagal mengunduh FFmpeg.\nError: {e}")));
        if os.path.exists(zip_path): os.remove(zip_path); self.update_queue.put(('destroy', None))

# ==============================================================================
# 3. FUNGSI BANTUAN (DENGAN PATH APPDATA)
# ==============================================================================
def get_app_support_path():
    """Mengarahkan path utama ke folder AppData/Roaming/UltimateDownloader."""
    base_path = os.path.join(os.environ.get('APPDATA'), "UltimateDownloader")
    os.makedirs(base_path, exist_ok=True)
    return base_path

def get_ytdlp_path():
    """Mendapatkan path ke yt-dlp.exe di dalam AppData."""
    return os.path.join(get_app_support_path(), "bin", "yt-dlp.exe")

def find_ffmpeg():
    """Mencari ffmpeg.exe, prioritas di AppData, lalu di PATH sistem."""
    local_path = os.path.join(get_app_support_path(), "bin", "ffmpeg.exe")
...
class App(customtkinter.CTk):
    def __init__(self):
        super().__init__()
        self.current_lang = "id"; self.texts = LANGS[self.current_lang]
        self.title(self.texts["app_title"]); self.geometry("600x520"); self.resizable(False, False)
        ...
        self.main_frame = customtkinter.CTkFrame(self, corner_radius=15); self.main_frame.pack(pady=20, padx=20, fill="both", expand=True)
        self.title_label = customtkinter.CTkLabel(self.main_frame, font=customtkinter.CTkFont(size=20, weight="bold")); self.title_label.pack(pady=(10, 10))
        self.url_entry = customtkinter.CTkEntry(self.main_frame, width=400); self.url_entry.pack(pady=10, padx=20)
        self.options_frame = customtkinter.CTkFrame(self.main_frame, fg_color="transparent"); self.options_frame.pack(pady=10, padx=20, fill="x"); self.options_frame.columnconfigure((0, 1), weight=1)
        self.format_label = customtkinter.CTkLabel(self.options_frame); self.format_label.grid(row=0, column=0, padx=5, pady=5, sticky="e")
        self.format_var = customtkinter.StringVar(); self.format_segmented_button = customtkinter.CTkSegmentedButton(self.options_frame, variable=self.format_var, command=self.on_format_change); self.format_segmented_button.grid(row=0, column=1, padx=5, pady=5, sticky="ew")
        self.quality_label = customtkinter.CTkLabel(self.options_frame); self.quality_label.grid(row=1, column=0, padx=5, pady=5, sticky="e")
        self.quality_options = ["", "1080p", "720p", "480p"]; self.quality_menu = customtkinter.CTkOptionMenu(self.options_frame, values=self.quality_options); self.quality_menu.grid(row=1, column=1, padx=5, pady=5, sticky="ew")
        self.download_button = customtkinter.CTkButton(self.main_frame, command=self.start_download); self.download_button.pack(pady=20, padx=20, ipady=5)
        self.status_frame = customtkinter.CTkFrame(self.main_frame, fg_color="transparent"); self.progress_bar = customtkinter.CTkProgressBar(self.status_frame, width=300); self.progress_bar.set(0); self.status_percent_label = customtkinter.CTkLabel(self.status_frame, text="0%"); self.status_process_label = customtkinter.CTkLabel(self.status_frame, text="", wraplength=500)
        self.main_status_label = customtkinter.CTkLabel(self.main_frame, text="", wraplength=500, justify="center"); self.main_status_label.pack(pady=(0, 10), fill="x", expand=True)
        self.footer_frame = customtkinter.CTkFrame(self, fg_color="transparent"); self.footer_frame.pack(side="bottom", fill="x", padx=10, pady=(5, 10))
        self.about_button = customtkinter.CTkButton(self.footer_frame, text="i", font=customtkinter.CTkFont(size=15, weight="bold"), width=28, height=28, command=self.show_about_window); self.about_button.pack(side="left", padx=10)
        self.lang_menu = customtkinter.CTkOptionMenu(self.footer_frame, values=["Indonesia", "English"], command=self.change_language, width=120); self.lang_menu.pack(side="right", padx=10)
        self.update_ui_texts(); self.on_format_change(self.format_var.get()); self.process_queue()
        ...
        try:
            while True:
                ...
                if msg_type == 'progress': self.progress_bar.set(value)
                    ...
                    self.status_frame.pack_forget(); status_text, final_path = value; self.main_status_label.configure(text=f"{status_text}\nDisimpan di: {os.path.dirname(final_path)}"); self.download_button.configure(state="normal", text=self.texts["download_button"])
                elif msg_type == 'error':
                    self.status_frame.pack_forget(); self.main_status_label.configure(text=self.texts["ready_status"]); self.download_button.configure(state="normal", text=self.texts["download_button"]); messagebox.showerror(self.texts["err_download_title"], self.texts["err_download_msg"].format(value))
                elif msg_type == 'run_updater': self.run_ytdlp_update()
        except queue.Empty: pass
        self.after(100, self.process_queue)
    ...
    def start_download(self):
        ...
        self.download_button.configure(state="disabled", text=self.texts["processing_button"]); self.main_status_label.configure(text=""); self.status_frame.pack(pady=10); self.progress_bar.pack(pady=(5,0)); self.status_percent_label.pack(); self.status_process_label.pack()
        chosen_format = self.format_var.get(); chosen_quality = self.quality_menu.get(); threading.Thread(target=self.download_video_thread, args=(self.url_entry.get(), save_path, chosen_format, chosen_quality)).start()

    def download_video_thread(self, video_url, save_path, chosen_format, chosen_quality):
        ytdlp_path = get_ytdlp_path()
        if not os.path.exists(ytdlp_path): self.update_queue.put(('run_updater', None)); threading.Event().wait(1); 
        if not os.path.exists(ytdlp_path): self.update_queue.put(('error', "yt-dlp.exe tidak ada. Coba perbarui lagi.")); return
        ...
        if chosen_format == self.texts["format_audio"]:
            if not ffmpeg_path: self.update_queue.put(('error', self.texts["ffmpeg_warn_msg"])); return
            command.extend(['-x', '--audio-format', 'mp3', '--audio-quality', '192K'])
        else:
            quality_map = { self.texts["quality_best"]: 'bestvideo*+bestaudio/best', "1080p": 'bestvideo[height<=1080]+bestaudio', "720p": 'bestvideo[height<=720]+bestaudio', "480p": 'bestvideo[height<=480]+bestaudio' }
            command.extend(['-f', quality_map.get(chosen_quality, quality_map[self.texts["quality_best"]])])
        
        if ffmpeg_path: command.extend(['--ffmpeg-location', os.path.dirname(ffmpeg_path)])
        command.extend(['-o', output_template, '--no-playlist', '--progress', '--progress-template', '{"status":"%(progress.status)s", "downloaded_bytes":%(progress.downloaded_bytes)d, "total_bytes":%(progress.total_bytes)d, "speed":"%(progress.speed)s"}', video_url])
        ...
        try:
            ...
            process.wait()
            if process.returncode == 0: self.update_queue.put(('finished', (self.texts["finished_status"], output_template)))
            else: self.update_queue.put(('error', process.stderr.read()))
        except Exception as e: self.update_queue.put(('error', e))
    
    def change_language(self, choice): self.current_lang = "id" if choice == "Indonesia" else "en"; self.update_ui_texts()
    def run_ytdlp_update(self): root = customtkinter.CTk(); root.withdraw(); updater_window = DownloaderWindow(root); updater_window.start_ytdlp_update(); root.wait_window(updater_window)
    def show_about_window(self):
        if hasattr(self,'about_win') and self.about_win.winfo_exists(): self.about_win.focus(); return
        self.about_win = customtkinter.CTkToplevel(self); self.about_win.title("Tentang Aplikasi"); self.about_win.geometry("350x300"); self.about_win.resizable(False, False); self.about_win.transient(self); self.about_win.grab_set(); self.about_win.iconbitmap(resource_path("favicon.ico"))
        customtkinter.CTkLabel(self.about_win, text="Downloader", font=customtkinter.CTkFont(size=18, weight="bold")).pack(pady=(20, 5))
        customtkinter.CTkLabel(self.about_win, text="Versi 1.5.0", font=customtkinter.CTkFont(size=12)).pack(pady=0)
    ...
    def update_ui_texts(self):
        ...
        if current_format in [LANGS['id']['format_video'], LANGS['en']['format_video'], ""]: self.format_var.set(video_text)
    ...
    def on_format_change(self, value):
        if value == self.texts["format_video"]: self.quality_label.grid(); self.quality_menu.grid(); self.quality_menu.configure(values=self.quality_options); self.quality_menu.set(self.texts["quality_best"])
        else: self.quality_label.grid(); self.quality_menu.grid(); audio_options = [self.texts["audio_quality_best"],self.texts["audio_quality_good"],self.texts["audio_quality_std"]]; self.quality_menu.configure(values=audio_options); self.quality_menu.set(self.texts["audio_quality_best"])
